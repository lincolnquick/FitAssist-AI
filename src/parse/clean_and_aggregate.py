...
def clean_and_aggregate(parsed_data: dict, weight_unit: str = "kg") -> pd.DataFrame:
    ...
    logger.debug(f"Top-level keys in parsed data: {list(data.keys())}")

    # Convert to per-metric DataFrames
    metric_dfs = {}
    ...
    for hk_type, label in REQUIRED_METRICS.items():
        if hk_type not in metric_dfs:
            ...
            continue
        df = metric_dfs[hk_type].drop_duplicates(subset=["date", "value"])
    ...
    # Heuristic: prevent erroneous conversion if weights are already in kg
    if weight_unit == "lb":
        avg_weight = daily_df["Weight"].mean()
        if avg_weight > 130:  # likely in pounds
            daily_df["Weight"] *= LBS_TO_KG
        if "LeanBodyMass" in daily_df and daily_df["LeanBodyMass"].mean() > 130:
            daily_df["LeanBodyMass"] *= LBS_TO_KG

    # Convert distance to km from m
